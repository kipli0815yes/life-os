...
function loadDB() {
  try {
    ...
    // migrate: add doneDate to existing tasks
    const migrate = tasks => tasks.forEach(t => { if (t.doneDate===undefined) t.doneDate=null; });
    if (s.confs)  s.confs.forEach(c  => migrate(c.tasks||[]));
    if (s.papers) s.papers.forEach(p => migrate(p.tasks||[]));
...
async function fetchCal() {
  ...
  if(sd)sd.className='sdot loading'; if(st)st.textContent='加载中…';
  // Prefer Apps Script (real-time, no proxy)
  if(DB.settings.gasUrl){
    try{
      const res=await fetch(DB.settings.gasUrl+'?action=calendar');
      const data=await res.json();
      if(data.status==='ok'&&Array.isArray(data.events)){
        calEvents=data.events.map(e=>({summary:e.title,start:e.start?new Date(e.start):null,end:e.end?new Date(e.end):null,allDay:e.allDay}));
        if(sd)sd.className='sdot ok'; if(st)st.textContent=`Google Calendar 已连接 · ${calEvents.length}个事件`;
        return;
      }
    }catch(e){/* fall through to ICS */}
  }
  // Fallback: ICS via proxy
  const url=DB.settings.icsUrl||DEFAULT_ICS;
  try{
    const res=await fetch(ICS_PROXY+encodeURIComponent(url));
    const text=await res.text();
    calEvents=parseICS(text);
    if(sd)sd.className='sdot ok'; if(st)st.textContent=`Google Calendar 已连接（ICS）· ${calEvents.length}个事件`;
  }catch(e){if(sd)sd.className='sdot err';if(st)st.textContent='Calendar 连接失败（请在设置里配置 Apps Script URL）';}
}
...
function renderMonth(){
  ...
  for(let d=1;d<=last;d++){
    const day=new Date(y,m,d),isToday=day.toDateString()===TODAY.toDateString();
    const items=[];
    getCalEventsForDay(day).forEach(e=>items.push({label:e.summary,type:'cal'}));
    getSysTasksForDay(day).forEach(t=>items.push({label:t.label,type:'task'}));
    if(!items.length&&!isToday)continue;
    const dEl=document.createElement('div'); dEl.className='week-day';
    const dstr2=`${y}-${String(m+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
    dEl.innerHTML=`<div class="wdh${isToday?' today':''}"><span style="font-size:14px;font-weight:600">${m+1}/${d}</span>${isToday?'<span style="font-size:11px;padding:2px 8px;border-radius:10px;background:var(--red-bg);color:var(--red)">今天</span>':''}<span class="day-add-btn" data-action="day-add" data-date="${dstr2}"><i class="ti ti-plus" style="font-size:11px"></i>加任务</span></div>`;
    items.forEach(item=>dEl.innerHTML+=`<div class="wi ${item.type==='cal'?'cal':'task'}"><div class="wdot" style="background:${item.type==='cal'?'var(--pur)':'var(--blu)'}"></div><span style="font-size:13px">${esc(item.label)}</span></div>`);
    mc.appendChild(dEl);
  }
}
...
document.addEventListener('click',e=>{
  ...
  const freqBtn=t.closest('[data-freq]');if(freqBtn){routineFreq=freqBtn.dataset.freq;document.querySelectorAll('[data-freq]').forEach(el=>el.classList.toggle('sel',el===freqBtn));document.getElementById('rout-weekly').style.display=routineFreq==='weekly'?'block':'none';if(routineFreq==='weekly'&&!routineDays.length)reassignDays();return;}
...
function setupRoutineToggle(){
  ...
  document.getElementById('rout-weekly').style.display='none';
  buildRoutDayPills();
}
...
async function callGemini(prompt){
  ...
  if(!key){alert('请先在「设置」里填入 Gemini API Key（免费申请：aistudio.google.com/apikey）');return null;}
  const url=`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${key}`;
  try{
    const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:prompt}]}]})});
    const data=await res.json();
    if(data.error)throw new Error(data.error.message);
    return data.candidates?.[0]?.content?.parts?.[0]?.text||'（AI没有返回内容）';
